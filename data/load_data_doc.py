...
#def process(line, redisd, relations, dev_docs):
def process(line):
...
def main(base_model):
    ...
    with Pool(16, initializer=initializer, initargs=(base_model, relations, dev_docs)) as p:
    ...
    for line in tqdm(lines):
        instances, is_dev = process(line, redisd, relations, dev_docs)
        if is_dev:
            dev_examples.extend(instances)
        else:
            train_examples.extend(instances)
